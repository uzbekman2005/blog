...
import (
	...
	"gitlab.com/azizbekdev-blog/go-monolithic/storage/postgres/errorhandler"
)

func (r *AboutRepo) Create(ctx context.Context, req *CreateReq) (*FullResponse, error) {
	res := &FullResponse{}
	query := r.Db.Builder.Insert("about_me").Columns(
		"title, intro, resume_link, linkedin, youtube, faceebook, telegram",
	).Values(req.Title, req.Intro, req.ResumeLink, req.LinkedIn, req.Youtube, req.Facebook, req.Telegram).Suffix(
		"RETURNING id, title, intro, resume_link, linkedin, youtube, faceebook, telegram")

	err := query.RunWith(r.Db.Db).Scan(
		&res.Id, &res.Title, &res.Intro, &res.ResumeLink,
		&res.LinkedIn, &res.Youtube, &res.Facebook, &res.Telegram,
	)
	if err != nil {
...
func (r *AboutRepo) FindOne(ctx context.Context, req *FindOneReq) (*FullResponse, error) {
	query := r.Db.Builder.Select("id, title, intro, resume_link, linkedin, youtube, faceebook, telegram").
		From("about_me").
...
func (r *AboutRepo) Update(ctx context.Context, req *UpdateReq) (*FullResponse, error) {
	...
	query := r.Db.Builder.Update("about_me").SetMap(mp).
		Where(squirrel.Eq{"id": req.Id}).
		Suffix("RETURNING id, title, intro, resume_link, linkedin, youtube, faceebook, telegram")
